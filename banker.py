...
def Resource_Request(requist, process, Need, avail, alloca):
    for i in range(len(requist)):
        if requist[i] > Need[process][i]:
            print(f'Error condition ,the system must wait, since  Need {
                  Need[process]} of P{process} < Request {requist}')
            return False, False, False
        if requist[i] > avail[i]:
            print(
                f'Error condition ,the system must wait, since Available {avail} of P{process} < Request {requist} ')
            return False, False, False
